...
public class ProjectService {
    ...
    public ProjectService(ProjectRepository projectRepository, ProjectMemberRepository projectMemberRepository, UserRepository userRepository, AuditLogService auditLogService) {
    ...
    public void addMember(UUID projectId, ProjectMemberRequestDTO dto, User authenticatedUser) {
        ...
        // Se o usuário que está tentando associar alguém for diferente do alvo, ele deve ser ADVISOR do projeto.
        if (!targetUserId.equals(authenticatedUser.getId())) {
            ProjectMember advisorMember = projectMemberRepository.findByProjectIdAndUserId(projectId, authenticatedUser.getId())
                    .orElseThrow(() -> new RuntimeException("User is not part of the project to add members"));
