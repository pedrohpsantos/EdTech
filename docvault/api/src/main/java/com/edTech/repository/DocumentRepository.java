...
public interface DocumentRepository extends JpaRepository<Document, UUID> {
  ...
  @Query(
      "SELECT d FROM Document d JOIN ProjectMember pm ON d.project.id = pm.project.id WHERE pm.user.id = :userId")
  ...
  @Query(
      "SELECT COUNT(d) FROM Document d JOIN ProjectMember pm ON d.project.id = pm.project.id WHERE pm.user.id = :userId")
  long countDocumentsByUserId(@Param("userId") UUID userId);

  @Query(
      "SELECT COUNT(d) FROM Document d JOIN ProjectMember pm ON d.project.id = pm.project.id WHERE pm.user.id = :userId AND d.status = :status")
