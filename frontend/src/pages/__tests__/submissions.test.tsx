...
vi.mock('../../components/DatasetPreview', () => ({
  default: ({ url, type }: any) => (
    <div data-testid="dataset-preview" data-url={url} data-type={type}>
      Dataset
    </div>
  ),
}));
...
describe('Submissions Page', () => {
  ...
  it('opens and closes analysis modal with PDF', async () => {
    (apiServices.getDownloadUrl as any).mockResolvedValue({
      sucesso: true,
      dados: { downloadUrl: 'http://pdf-url' },
    });
  ...
  it('opens analysis modal with DatasetPreview for CSV', async () => {
    (apiServices.getDownloadUrl as any).mockResolvedValue({
      sucesso: true,
      dados: { downloadUrl: 'http://csv-url' },
    });
    render(<Submissions />);
  ...
  it('approves document from analysis modal', async () => {
    ...
    await waitFor(() =>
      expect(container.querySelector('div[class*="modalContent"]')).toBeInTheDocument(),
    );
  ...
  it('handles approve document error', async () => {
    ...
    (apiServices.reviewDocument as any).mockResolvedValue({
      sucesso: false,
      mensagem: 'Error approving',
    });
  ...
  it('rejects document from analysis modal', async () => {
    ...
    const feedbackInput = screen.getByPlaceholderText(
      'Descreva os ajustes necessários ao pesquisador...',
    );
