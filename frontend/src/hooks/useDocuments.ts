...
export const useDocuments = (projectId?: string, title?: string, page = 0, size = 20) => {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await getDocuments(projectId, title, page, size);
...
export const useUploadDocument = () => {
    ...
    return useMutation({
        mutationFn: async ({ file, title, projectId, onProgress }: { file: File, title: string, projectId: string, onProgress?: (p: any) => void }) => {
...
export const useDownloadUrl = () => {
    return useMutation({
        mutationFn: async (documentId: string) => {
            ...
            if (!res.sucesso) throw new Error(res.mensagem);
            return typeof res.dados === 'string' ? res.dados : (res.dados?.url || res.dados?.downloadUrl || res.dados?.fileUrl);
        }
    });
};
