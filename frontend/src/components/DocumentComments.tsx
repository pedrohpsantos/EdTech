  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f8fafc', borderLeft: '1px solid #e2e8f0' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {isLoading ? (
          <div style={{ textAlign: 'center', color: '#64748B', fontSize: '13px' }}>Carregando...</div>
        ) : comments.length === 0 ? (
          ...
          comments.map((comment: any) => (
            <div key={comment.id} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontWeight: 600, fontSize: '13px', color: '#334155' }}>{comment.authorName}</span>
                <span style={{ fontSize: '11px', color: '#94a3b8' }}>
                  {new Date(comment.createdAt).toLocaleDateString()} {new Date(comment.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
              <div style={{ background: 'white', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '13px', color: '#475569', boxShadow: '0 1px 2px rgba(0,0,0,0.02)', wordBreak: 'break-word' }}>
                {comment.content}
              </div>
            </div>
          ))
      ...
      <div style={{ padding: '16px', background: 'white', borderTop: '1px solid #e2e8f0' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '8px' }}>
          ...
          <button 
            ...
            style={{ background: 'var(--ed-purple)', color: 'white', border: 'none', borderRadius: '6px', padding: '0 16px', cursor: (!content.trim() || isPending) ? 'not-allowed' : 'pointer', opacity: (!content.trim() || isPending) ? 0.6 : 1 }}
          >
