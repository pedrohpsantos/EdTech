...
Locator: getByText(/Erro ao realizar o login/i).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/Erro ao realizar o login/i).first()
...
```ts
  ...
  73 |     const errorMessage = page.getByText(/Erro ao realizar o login/i);
> 74 |     await expect(errorMessage.first()).toBeVisible({ timeout: 5000 });
  ...
  86 |       await page.waitForURL('**/recover-password*');
  87 |       
