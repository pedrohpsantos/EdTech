...
test.describe("Production Smoke Test", () => {
  ...
  const API_URL =
    process.env.API_URL || "https://edtech-backend-shv6qbpf4q-rj.a.run.app";

  test("A aplicação frontend está acessível e HTTPS (TLS) é forçado", async ({
    page,
  }) => {
  ...
  test("Rotas protegidas bloqueiam acesso e redirecionam para o Login", async ({
    page,
  }) => {
  ...
  test("Validar elementos visuais principais da página de Login", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Bem-vindo|Login|Entrar/i }),
    ).toBeVisible();
  ...
  test("Tentativa de login com credenciais inválidas exibe alerta", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/login`);
  ...
  test("Acesso à tela de Recuperação de Senha funciona corretamente", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/login`);
