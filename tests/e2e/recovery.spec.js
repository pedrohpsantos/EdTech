...
test.describe("Fluxo de Recuperação de Senha (E2E)", () => {
  test("Deve realizar o fluxo completo de esqueci minha senha", async ({
    page,
  }) => {
    ...
    await page.route("**/api/auth/recovery/request", (route) => {
      route.fulfill({
        ...
        body: JSON.stringify({
          sucesso: true,
          mensagem: "Código enviado para o seu e-mail (se cadastrado).",
        }),
      });
    });
    ...
    await expect(
      page.locator("text=Senha redefinida com sucesso! Redirecionando..."),
    ).toBeVisible();
  });
});
