...
const reportJsonPath = resolve(reportDirectory, 'production-security-contract.json');
...
const run = async () => {
  ...
  if (response) {
    ...
    const headerChecks = [
      ['HTTP Strict Transport Security', 'strict-transport-security', 'max-age=31536000'],
      ['HSTS includes subdomains', 'strict-transport-security', 'includesubdomains'],
      ['MIME sniffing protection', 'x-content-type-options', 'nosniff'],
      ['Clickjacking protection', 'x-frame-options', 'deny'],
      ['Content Security Policy frame boundary', 'content-security-policy', "frame-ancestors 'none'"],
    ...
    for (const [name, header, expected] of headerChecks) {
      ...
      checks.push({ name, passed: hasValue(headers, header, expected), expected, actual });
    }
    const traceId = headers.get('x-request-id') || '(header missing)';
    checks.push({ name: 'Request tracing correlation', passed: traceId === requestId, expected: requestId, actual: traceId });
  }

  const report = { suite: 'EdTech API Security Contract', endpoint: `${apiUrl}/actuator/health`, executedAt: startedAt.toISOString(), durationMs: Date.now() - startedAt.getTime(), passed: checks.length > 0 && checks.every((check) => check.passed), checks };
  await mkdir(reportDirectory, { recursive: true });
  await Promise.all([writeFile(reportJsonPath, `${JSON.stringify(report, null, 2)}\n`), writeFile(reportHtmlPath, renderReport(report))]);
  for (const check of checks) console.log(`${check.passed ? 'PASS' : 'FAIL'} ${check.name}: ${check.actual}`);
