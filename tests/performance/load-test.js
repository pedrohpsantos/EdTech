...
export default function () {
  ...
  check(loginResponse, {
    "login respondeu (banco acessado)": (res) =>
      res.status === 401 || res.status === 200 || res.status === 429,
  });
  ...
  const registerPayload = JSON.stringify({
    ...
    password: __ENV.LOAD_TEST_REGISTER_PASSWORD,
    role: "RESEARCHER",
  });
  const registerResponse = http.post(
    `${baseUrl}/api/auth/register`,
    registerPayload,
    { ...params, tags: { endpoint: "register" } },
  );
  check(registerResponse, {
    "registro processado (2xx, 400 ou 429)": (res) =>
  ...
  const recoveryResponse = http.post(
    `${baseUrl}/api/auth/recovery/request`,
    recoveryPayload,
    { ...params, tags: { endpoint: "recovery" } },
  );
  check(recoveryResponse, {
    "recuperação processada (2xx, 404 ou 429)": (res) =>
      res.status >= 200 && res.status < 500,
  });
